...
| Natural-language planning | Sends Chinese research intent through the active, tested model and prompt version. The model returns only a closed `collection_intent_v1`; Sortlytic validates the intent, translates keyword searches into the target region's primary query language, preserves direct IDs and URLs, and deterministically builds the executable `collection_plan_v4`. |
...
AI configuration is optional for form-based planning and required for natural-language planning and prompt regression. The active prompt content, version, output Schema, and evidence constraints are sent to the selected model and recorded in the runtime snapshot. Sortlytic validates the raw model JSON against the closed `collection_intent_v1` contract locally. The model may extract intent and translate keyword searches, but it cannot choose endpoints, steps, dependencies, request parameters, pagination, enrichment, or cost. The Rust backend derives those details from the capability catalog, adds evidence-based region and time filtering, validates endpoint allowlists and budget rules, and produces the final `collection_plan_v4`. Missing, ambiguous, untranslated, or incompatible intent remains in review state with actionable diagnostics instead of changing task scope or allowing execution.
