...
pub fn normalize_account_with_evidence(
  ...
  if let Some(raw_region) = account_fields.get("country_region").and_then(Value::as_str) {
    if let Some(region) = normalize_country_region(Some(raw_region)) {
      account_fields.insert("country_region".to_string(), Value::String(region));
    } else {
      account_fields.remove("country_region");
      field_evidence.remove("country_region");
    }
  }
...
pub(crate) fn normalize_country_region(value: Option<&str>) -> Option<String> {
