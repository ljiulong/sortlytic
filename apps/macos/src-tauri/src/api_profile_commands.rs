...
mod tests {
  ...
  fn mutate_tikhub(
    root: &std::path::Path,
    profile: &crate::api_profiles::TikhubApiProfile,
    operation: &str,
  ) -> AppResult<()> {
    if operation == "edit" {
  ...
  fn active_runtime_snapshot_blocks_tikhub_edit_and_delete() {
    ...
    insert_snapshot(&connection, profile, "running", "snapshot", &profile.id);
    drop(connection);
  ...
  fn claim_transaction_serializes_tikhub_edit_and_delete() {
    for operation in ["edit", "delete"] {
      let root = workspace(&format!("claim-race-{operation}"));
      let registry = service::save_profile(
        &root,
        tikhub_input(
          None,
      ...
      let before = fs::read(root.join("secrets/api-config.json")).unwrap();
      let mut connection = open_workspace_database(root.join(DATABASE_FILE_NAME)).unwrap();
      let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .unwrap();
      insert_snapshot(&transaction, &profile, "running", "claim-race", &profile.id);
      ...
      thread::spawn(move || {
        ...
        let result = mutate_tikhub(&worker_root, &worker_profile, operation);
  ...
  fn claim_cannot_enter_between_mutability_check_and_registry_write() {
    for operation in ["edit", "delete"] {
      let root = workspace(&format!("claim-gap-{operation}"));
      let registry = service::save_profile(
        &root,
        tikhub_input(
          None,
      ...
      if let Some(transaction) = transaction.as_ref() {
        insert_snapshot(transaction, &profile, "running", "gap", &profile.id);
      }
  ...
  fn legacy_runtime_snapshots_block_profiles_by_stable_credential_reference() {
    for status in ["queued", "running"] {
      ...
      let profile = registry.tikhub_profiles.values().next().unwrap();
      let connection = open_workspace_database(root.join(DATABASE_FILE_NAME)).unwrap();
      insert_snapshot(&connection, profile, status, "legacy-snapshot", "default");
      drop(connection);
