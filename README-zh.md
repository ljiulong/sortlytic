...
| 自然语言计划 | 使用当前已测试的模型与已激活提示词处理中文研究意图。模型只返回闭合的 `collection_intent_v1`；Sortlytic 校验意图、把关键词翻译为目标地区主检索语言、原样保留直接 ID 与 URL，再由后端确定性生成可执行的 `collection_plan_v4`。 |
...
打开**设置 → 配置 AI API**，可以保存多条 OpenAI、Anthropic、Gemini、自定义 OpenAI-compatible 或 Ollama 配置，并在脱敏列表中查看和切换当前配置。选择 API 格式，按需填写 Base URL，再填写默认模型 ID 和 API Key，然后点击**保存并测试**。测试会向真实模型端点发送最小请求，并要求返回闭合结构 `{"ok":true}`，成功后配置才能成为当前配置。Ollama 可以不填密钥。官方供应商类型只能使用对应官方 HTTPS 主机；第三方 OpenAI-compatible 网关必须选择自定义类型并使用 HTTPS；本地 Ollama 仅可在回环地址使用 HTTP。编辑其他字段时可以留空复用已保存 Key，但供应商或 URL authority（协议、主机、端口）变化时必须重新输入。

表单式计划可以不配置 AI；自然语言计划和提示词回归必须使用当前已测试的 AI 配置。当前激活提示词的正文、版本、输出 Schema 与证据约束会真实进入模型请求并写入运行快照。Sortlytic 会在本地对原始模型 JSON 执行闭合的 `collection_intent_v1` 校验。模型只负责提取意图和翻译关键词，不能选择端点、步骤、依赖、请求参数、分页、补全或成本；Rust 后端根据能力目录确定这些执行细节，加入基于明确证据的地区与时间筛选，校验端点白名单和预算规则，并生成最终 `collection_plan_v4`。字段缺失、语义歧义、检索词未翻译或能力不兼容时，任务会保留在待修正状态并显示可操作诊断，不会改写任务范围或进入执行队列。
...
| 自然语言 | 希望用中文描述调研目标，再由当前模型配置和已激活提示词整理成结构化计划。 | 检查推断的平台、数据类型、筛选条件、缺失条件、记录上限、预算和模型证据。 |
...
单任务记录数范围为 10～5,000 条筛选合格且去重后的账号行，成本上限可填写 0.1～500。账号优先按“平台 + 平台用户 ID”去重，缺少稳定 ID 时回退到规范化账号标识。
...
打包版本可以通过**设置 → 关于 Sortlytic**更新：

1. 点击**检查更新**。
2. 查看版本号与更新说明。
3. 点击**下载并安装**。Sortlytic 会校验 Tauri updater 更新产物签名并准备已安装更新，但不会自动重启。
4. 更新准备完成后，点击**重启并更新**。
