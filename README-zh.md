...
| 自然语言计划 | 使用当前已测试的模型配置和已激活提示词处理中文研究意图，在保存计划与运行快照前校验 `collection_plan_v3` Schema 和业务规则。 |
...
表单式计划可以不配置 AI；自然语言计划和提示词回归必须使用当前已测试的 AI 配置。当前激活提示词的正文、版本、输出 Schema 与证据约束会真实进入模型请求并写入运行快照。Sortlytic 会在本地对原始模型 JSON 执行闭合的 `collection_plan_v3` 校验，再执行业务与端点规则；无效输出只会进入复核状态，不会改写任务范围或进入执行队列。
